...
| 5 | Store-queue synthesis A/B ([sq/spec.md §6.5](sq/spec.md), experiment 1) | ECP5 synth of a queue block | a store queue | Decides the scrub mechanism; no bar item |
...
| 8 | FP register-file synthesis A/B ([fpu/spec.md §7.7](fpu/spec.md), experiment 1) | An FPU register file | a Tier-1 FPU and SIMD unit | Decides the scrub mechanism; no bar item |
...
| **L2** — IOMMU default-deny | `NOT MET` | Items 19–25, after §4 programme 4 — and the prerequisite is the **bus**, not the IOMMU | Items 30–32. The GPU's windows sit inside the GPU, one master port below where this item acts. What the GPU *does* do is flip this item from `N/A`-as-blocker to blocking, by being the first tenant-influenced DMA master |
| **L3** — eager FP/vector switch | `NOT MET` | The five residue tests of [fpu/spec.md §7.7](fpu/spec.md) and [simd/spec.md §2.6.1](simd/spec.md), after §4 programme 2 — **and the no-saved-image branch must be its own case**, because a test between two established owners passes without touching the defect this item names | Items 8–10 on their own. They decide *which* scrub mechanism is built and what it costs; none of the three is a residue demonstration. Nor does the vertical-FP ownership fix: Its defect is between two tasks inside one guest, and this item's boundary is the tenant |
...
| **L5** — cache isolation beyond ways | `NOT MET` | Items 12–16, after §4 programme 3, and even then only per sub-clause: five mechanisms, five tests, and a single "partitioning works" result discharges the first only. Item 17 is a build-refusal guard and item 18 a performance gate; neither is one of the five | **The user-flush finding does not move it, and this is the worked example.** F2 found that the merged kernel cache-maintenance change turned `sys_cacheflush(2)` into a real flush primitive. Both reasons this item is unmet concern an L2 that does not exist, so a finding about an L1 that does neither opens nor closes a sub-clause |
| **L6** — scrub on ownership change | `NOT MET` | **Item 15 and nothing else in §1's list.** L6's bar is a *residue test per site*, and only `P-E4` is one; the store-queue, FP, SIMD and GPU sites are discharged by the residue tests §7 names, which have no numbered item because they are evidence obligations rather than measurements. Programmes 1, 2, 3 and 6 unblock them | **The synthesis A/Bs do not move it.** Items 5 and 8 price a scrub mechanism and may change which one is built; neither is a residue demonstration. Nor do items 30–32, which measure the GPU's *window checkers* — its L6 sites are under the handover scrub, a different rule. And **closing the last open `undefined` site moved nothing**, and this is the trap most likely to be re-fallen into. That count reaching zero completes the *specification* half; the bar is a residue test per site demonstrated red before the fix, and there is no store queue, no FPU, no SIMD unit, no GPU and no L2 to run one red on |
| **L7** — the walker's data source is hypervisor-owned | `NOT MET` | Hypervisor Phase 3. No item in §1's list is against this | Items 1 and 2. The task that specified them widened part 4 of this item to a second counter and did not move it: both counter addresses are decoded in RTL and nothing virtualizes either |
