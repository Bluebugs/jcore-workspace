...
- **P4.** The SH-4 architectural privileged-MMIO region (`0xE0000000`–`0xFFFFFFFF`). All on-chip control registers live here. Within P4, the range `0xE0000000`–`0xEFFFFFFF` is the **Store Queue (SQ)** region, which is *not* MMIO in the ordinary sense (see **Store Queue** below). Allocation is governed by the canonical [soc/p4-mmio-map.md](soc/p4-mmio-map.md). Prior art: SH-4 hardware manual (Renesas, 1998).
