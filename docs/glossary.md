...
- **SR.VD — SIMD disable (SR bit 13).** J-Core SIMD extension; SR.FD's direct analogue for the V0..V15 + P0 + VCSR state. When set, any SIMD-touching instruction or control-register access raises SIMD-disabled exception. Enables the OS lazy-SIMD-context-switch idiom and avoids the 264-byte save/restore for tasks that never touch SIMD (~95% of typical Linux processes). On Tier 2 (hypervisor) implementations the trap is reported as `EXC_SIMD_DISABLED` (EXPEVT `0x1C0`, HEDR bit 24, delegatable). See [simd/spec.md §2.6](simd/spec.md). Prior art: PowerPC G4 AltiVec `MSR.VEC` (1999); SH-4 `SR.FD` (1998); Apple Mac OS X lazy AltiVec save.
