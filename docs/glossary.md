...
Practical consequence: where a mechanism's *purpose* is post-2006 even though its structure is not, the prior-art citation is necessary but a check for live claims on the purpose-specific combination is also required.

*This paragraph previously read "which in practice means security mechanisms", and closed "The security sections of the CPU specs are currently the only place this applies." Both are now false, and the second was the more misleading: [bi-endian-spec.md §10](bi-endian-spec.md) invokes rule 2 for a mechanism that is not a security mechanism at all — per-**context** endianness control is 1990s prior art, while per-**guest** endianness switched on VM entry and exit under a hypervisor is a purpose-specific combination whose purpose postdates the structures by a decade. Virtualization is the second such purpose, and there is no reason to expect it is the last. Read rule 2 as applying wherever the purpose is younger than the structure, and check rather than assume which sections those are.*
...
**The allocation rule above is enforced, not merely stated, on implementations that carry the tenancy check** ([hypervisor/hardware-spec.md §4.7.2](hypervisor/hardware-spec.md), Wave-3 C2c). **And "core" in it is the *property* stated in the paragraph above, not the word:** anything with several hardware contexts sharing unpartitioned lookup and predictor state is a core for this rule, which [security/threat-model.md §8](security/threat-model.md) **L1** applies to rule that the GPU's SM is one. That document owns the ruling; this entry owns only the term, and names it here because the term is what the ruling turns on.
...
- **Endianness.** The byte order of the core, and of the kernel and userspace built for it. One answer for the whole product line, not a per-product-point property — which is why it is a term here and not a column in §3. Owned by [platform-baseline.md §2](platform-baseline.md); the decision and the alternative it rejects are [decisions/0006](decisions/0006-endianness-is-big-endian.md). Not to be confused with SIMD **lane** order, which is a separate convention owned by [simd/spec.md §2.2](simd/spec.md), or with the **byte order a context runs in**, which J-Core is to make a per-context, hypervisor-owned mode covering both the data path and instruction fetch, per [bi-endian-spec.md](bi-endian-spec.md). *This entry previously said the mode covered a guest's **data** only, with instruction fetch staying big-endian; that was Decision B2-1 and it is superseded.*
- **Byte invariance / word invariance.** The two ways a machine can offer both byte orders, in ARM's terminology **BE-8** and **BE-32**. Under **byte invariance** a given address is the same memory byte in both modes and only the assembly of bytes into register values changes; under **word invariance** the hardware adjusts the low address bits of sub-word accesses, so a byte access reaches a different physical byte. J-Core is byte-invariant, and word invariance is prohibited rather than merely unused. Owned by [bi-endian-spec.md §2](bi-endian-spec.md), whose §11 carries the prior art and records which sources support which clause — the distinction matters here, because two of the obvious references teach the *other* scheme.
- **`LE` / `HLE`.** The two bits of the byte-order mode: the byte order a non-hyperprivileged context runs in, and the byte order hyperprivileged entry runs in. Neither is guest-writable or guest-readable. *Semantics: [bi-endian-spec.md §6](bi-endian-spec.md). Which register holds them and at which bit positions: unallocated, and recorded as an open item there.*
