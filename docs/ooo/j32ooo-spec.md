...
**Update policy**: all five structures (bimodal, gshare, chooser, BTB, RAS) are updated **only from committed instructions**. *(This read "all four" while naming five — corrected 2026-09-09, Wave-3 C2b. A miscount in the sentence that carries this design point's commit-only-training rule is worth fixing on sight: the rule is one of the three [../security/threat-model.md §7.3](../security/threat-model.md) credits the design with already having, and a reader reconciling four against five has to guess which structure is exempt.)* Squashed instructions leave no predictor state behind. The per-thread GHR keeps a speculative working copy for indexing plus a committed copy restored on squash — the standard speculative-history-repair arrangement, here doing double duty as the mechanism that prevents a squashed path from training the tables.
...
- Consequence: no line fill, no L2 lookup, no allocation-time replacement-state update and no coherence traffic is ever caused by a load that **misses** and does not commit. It closes the **fill/allocation** half of the cache covert channel of §20.2, at L1, at L2 and across the coherence fabric.
- **Scoped 2026-09-09, Wave-3 C2b.** This bullet previously read *"no line fill, no L2 lookup, no replacement-state update and no coherence traffic is ever caused by a load that does not commit … it closes it for L1, L2, replacement metadata and the coherence fabric simultaneously."* Two things were wrong with that sentence and they are different in kind. **(a) It contradicted the bullet above it.** Speculative hits proceed at full speed, and a hit *is* a replacement-state update: §11.1 and §11.2 make both L1s pseudo-LRU, so a hitting load that never commits moves a pLRU tree and changes which way of that set is evicted next. Two bullets of one subsection asserted that hits are unrestricted and that no non-committing load updates replacement state. **(b) It was a completeness claim**, and [../security/threat-model.md §7.3](../security/threat-model.md) strikes it on the evidence the plan already carries: Pensieve (ISCA 2023) model-checked delay-on-miss and found Spectre-like attacks against it, and Speculative Interference (ASPLOS 2021) leaks through the timing of *older* instructions with no speculative miss issued at all. Delay-on-miss is the primary D-side mitigation. It is not a solution, and §20.2's table is where the rest of the coverage is accounted for.
- **The speculative-hit metadata channel, stated rather than closed.** The mechanism is [../security/threat-model.md §7.6](../security/threat-model.md)'s, one level up: access patterns leak through metadata updates on *hitting* loads. Here the two domains are transient and architectural rather than two way-partitions, but the transmitter is the same tree. The only mechanism that would close it is to hold the pLRU update in the LQ entry and apply it at commit from the way the ROB entry already knows — the shape §3.2 uses for predictor training, moved to the replacement tree. **[j4-remediation-plan.md §E.10](../j4-remediation-plan.md) has no line for it.** Its replacement-metadata entry prices DAWG, which partitions metadata *between tenants* and does nothing about a squashed path inside one; nothing in that menu prices suppressing a hit-time update, so no figure from it is borrowed here and the cost is **unknown at this stage — needs measurement**. Whether to build it is a decision for whoever resumes this design point under [../decisions/0009](../decisions/0009-in-order-fgmt-is-the-default-path.md); C2b's remit was the sentence, and a mitigation invented here for a paused core would have competed for attention with §5.0a of [../mmu/hardware-spec.md](../mmu/hardware-spec.md), which is about hardware that ships.
- **None of this reaches the core that exists.** `jcore-cpu@origin/master`'s L1-I and L1-D are **direct-mapped** — `cache/cache_pkg.vhd` dimensions the tag, data and valid arrays by `cache_lines` alone, with no way index anywhere — so they hold no replacement state for a speculative hit to update, and a case-insensitive search of that tree for `plru` or `pseudo-lru` returns nothing. The contradiction above lived only in the specifications of design points nobody is building.
...
1. **A squash cancels microarchitectural work, not just architectural state.** In addition to flushing the ROB/IQ/LQ/SQ entries of §9.3, a squash deallocates MSHRs allocated by squashed loads whose fills have not yet returned, and cancels prefetches generated by squashed loads **and by squashed fetches** (§11.1a). Without this, §8.2a leaks through the MSHR and prefetch paths. *(Widened 2026-09-09, Wave-3 C2b: this rule previously said "loads" in both clauses. The next-line prefetcher of §11.1 is triggered by a fetch and not by a load, so the frontend half of the prefetch path was outside every clause of this section — which is the D-side-only scope [../security/threat-model.md §7.3](../security/threat-model.md) is about.)*
...
### 11.1a Wrong-path fetch, the next-line prefetcher, and what a squash reaches

[j4-remediation-plan.md §C2](../j4-remediation-plan.md) names the transmitter
this subsection answers: *"wrong-path fetch + the always-on I-prefetcher fill the
shared L2."* Both halves are properties of §11.1 above — the prefetcher is listed
there as **Yes (always)** — and of nothing that has been built:
`jcore-cpu@origin/master` has no L2 and no I-side prefetcher (its only `prefetch`
token in the cache RTL is a half-word indicator inside a 32-bit RAM read, and the
optional `prefetch` unit the testbenches reference has no entity anywhere in the
tree and is off by default). This is a rule for a design point, not a fix for a
regression.

**Why the existing rules do not reach it.** §9.4 rule 2 says no structure is
trained by a squashed instruction, and for a next-line prefetcher that is
**vacuous rather than satisfied**: there is no table, no confidence counter and
no history to train — line N is fetched, so line N+1 is requested. A rule stated
against a property the structure does not have leaves the structure uncovered
while reading as though it were covered. §11.4's page-boundary rule, which
§11.1 does inherit, bounds *where* a prefetch may go and says nothing about
*whose* fetch asked for it.

**The rule.** A next-line prefetch may be issued **only on behalf of a fetch that
has dispatched**, and any prefetch outstanding on behalf of a squashed fetch is
cancelled with its MSHR deallocated (§9.4 rule 1). The lead time this costs is
the fetch-to-dispatch distance, not the prefetcher's function: a 32-byte line
holds up to sixteen 2-byte instructions, so the prefetch of line N+1 still has
most of line N to cover its latency. It is the same shape as §8.2a and as
[../mmu/hardware-spec.md §5.0a](../mmu/hardware-spec.md) **W-R1** — a condition
on an existing arm, no new structure — and it is deliberately the same shape,
because a frontend defence built as a buffer is §20.7 rejection 1.

**What the rule does not reach, and why it is accepted.** The *demand* fill of
the wrong-path fetch itself is untouched. A fetch cannot wait for its own
dispatch, so the delay shape above has no analogue here, and both mechanisms that
would close it are refused elsewhere: a flush-on-switch filter cache is on
[j4-remediation-plan.md §E.10](../j4-remediation-plan.md)'s **don't build** list,
and a speculative fill buffer promoted at non-speculative is §20.7 rejection 1,
refused on live-patent grounds. It is therefore an accepted residual, recorded in
[../security/threat-model.md §10](../security/threat-model.md) — the identical
finding, for the identical reason, as
[../mmu/hardware-spec.md §5.0a](../mmu/hardware-spec.md) **W-R4** reaches on the
core that exists.
...
The next-line prefetcher of §11.1 is subject to the same page-boundary rule, and to §11.1a, which is where the three constraints above are shown *not* to reach it.
...
7. **Core-granular tenancy (§20.3)**: adopted deliberately, and until 2026-09-09 the weakest link in the security model, since it was enforced by a scheduler — the hypervisor's under virtualization, Linux's otherwise — rather than by hardware. *(Wave-3 **C2c** added a hardware backstop for the virtualized case: [hypervisor/hardware-spec.md §4.7.2](../hypervisor/hardware-spec.md) refuses an `HRTE` that would put two tenant numbers on one core's contexts. The unvirtualized case executes no `HRTE` and is unchanged, so this item stays open rather than closing.)* Gang scheduling ([hypervisor/hardware-spec.md §4.7.1](../hypervisor/hardware-spec.md)) recovers tenant density over time, so the cost is now scheduling latency and a cold cache per quantum rather than a halved tenant count. If a future product point needs mutually distrusting *concurrently resident* contexts, the levers are per-thread L1 way partitioning and per-thread predictor arrays, at a cost that would substantially undo the FGMT gain. Do not adopt it silently. Note the L2 is a separate problem with a separate answer already adopted: [cache/l2-spec.md §16.1](../cache/l2-spec.md).
