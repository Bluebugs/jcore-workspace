...
>
> **Five corrections to the text below, recorded rather than silently applied**, since three of them are to *this* section's own premises and two are to the evidence §E.1 supplies it.
>
...
>
> **(5) The "boots" half of the question is not demonstrated for J4 either.** `jcore-cpu@master` CI establishes that the J4 variant fits and closes timing on the 85F, and that its MMU behaves — through bare-metal simulation harnesses that link the **real** `linux@jcore` TLB-miss handler and `head_32.S` MMU-enable path. Nothing in that repository's workflows boots a kernel on J4. Boot-to-Linux is a Phase-1 goal with a partial evidence trail, not an achieved property, and this section's phrasing ("which core actually fits and boots") invites reading it as achieved.
>
