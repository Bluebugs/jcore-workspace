...
- **Q3. CLOSED 2026-09-07.** *Previously read:* "Is there an existing big-endian-only constraint we need to respect? … We should pick one and stick to it for this work. Big-endian is the path of least resistance; flag as a decision." The decision was taken by Wave-2 B1: the platform is big-endian at every product point ([platform-baseline.md §2](platform-baseline.md), [decisions/0006](../decisions/0006-endianness-is-big-endian.md)). This plan needs no byte-order work; the "MMU chips will be little-endian" expectation the question recorded did not survive contact with the RTL, the toolchain or the kernel defconfig.
