...
| Flush+Reload | **APPLIES**, and the Flush half is **not** the instruction this cell **used to read**: it said "`ocbi`/`ocbp` are user-mode ([cache/l2-spec.md §17.5](../cache/l2-spec.md))", and `jcore-cpu@origin/master` decodes neither (§8 **L5**). The user-mode Flush primitive that exists is `sys_cacheflush(2)`'s `CACHEFLUSH_I` — §10 item 19 | same | Needs shared memory; therefore couples to page dedup — **L5**. The instruction-side primitive needs none, but reaches only the L1-I |
...
| **C3** lower-severity tracked items *(design landed 2026-09-10; C3 is scheduled in no wave and is the last item of [j4-remediation-plan.md](../j4-remediation-plan.md))* | **none — and that is the row, not an omission** | C3's five items are argued against this document and **not one of them is gated on a bar item**, so nothing here moves and all seven stay `NOT MET`. A reader arriving with a C-number and finding a blank cell will assume the row is unfinished, which is why the reasoning is written out. Two of the five were *accepted*, which §8 has no vocabulary for — the bar is a list of things that must be **MET**, and "we decided not to do this" has no cell in it. That is why both accepts landed in §10 as items 17 and 18, beside the other things this design does not close. The clause most likely to be missed is that C3 **changed §5**: the denial-of-service non-guarantee now carries a two-part test, because §7.7a had been accepting an availability finding against a §5 that scoped availability out absolutely, and nothing reconciled them. Any future proposal to close an availability finding is measured against that test and not against a fresh argument. The second is that C3's interrupt work found a **cross-domain delivery**, not a wording difference — a host-owned device's interrupt reaching a running guest's `VBR` — and closed it at AIC2 ([`A2-R1`](../aic/aic2-spec.md)) because the CPU applying `HEDR` cannot see which source is which; a reviewer looking for that fix in a hypervisor document will not find it. The third is that **§11 grew by two rows** rather than shrinking: guest `SR.IMASK` virtualization, and three `aic2-spec.md` §5 descriptions C3's remit did not cover |
...
| **Guest `SR.IMASK` is unvirtualized, and no document says which way.** *(Found by C3, 2026-09-10.)* [aic/aic2-spec.md §3.4](../aic/aic2-spec.md) gates delivery on `PRIO[winner] > SR.IMASK` against the **physical** `SR.IMASK`, which a dispatched guest writes as an ordinary SH-4 SR field ([hypervisor/hardware-spec.md §2.1](../hypervisor/hardware-spec.md), [priv-arch/design-spec.md §3.2](../priv-arch/design-spec.md)). Nothing states whether a guest write is trapped, shadowed per vCPU, or applied to hardware, and nothing relates `jcore_vintc`'s `PRIO[*]` to it. **C3 closed the sharp half and left the rest**: [`A2-R1`](../aic/aic2-spec.md) plus the internal vector's `cpu_event_i_t.msk` mean a guest raising `IMASK` can no longer hold off *host-owned* sources, because those are not delivered to its TC and the HS ping bypasses the `IMASK` comparison. *(The spec's own "`IRL=15`" is **not** what buys that — SH-4 accepts on level > `IMASK`, so 15 against `IMASK = 15` is masked; `decode/decode_core.vhm`'s `( ibit < event_i.lvl or event_i.msk = '1' )` is the real gate, and `A2-R1` now requires the bit.)* What remains is intra-guest and cross-vCPU: two vCPUs of one guest, and a guest's own view of its virtual controller's priorities. That is inside one tenant, which is where **L1** puts observer and victim — the same position §10 items 7, 12 and 13 take, which is why it is filed here rather than made a bar item | Hypervisor Phase 3, jointly with [aic/aic2-spec.md §5](../aic/aic2-spec.md) |
| **Three of `aic2-spec.md` §5's descriptions were not unified by C3 and are named so the burn-down is visible.** *(C3, 2026-09-10.)* C3's remit was the *delegation* description and it closed that ([`A2-R1`](../aic/aic2-spec.md), §5.5's rewrite, and the `VBR + 0x600` correction — owner [priv-arch/design-spec.md §4.5](../priv-arch/design-spec.md)). Three neighbouring descriptions are still stated more than one way, and each needs a decision C3 does not own. **(a)** What a guest's access to the physical AIC2 MMIO does has four answers: the emulated-MMIO trap `0x1E0` at `VBR_HYP + 0x200` ([hypervisor/hardware-spec.md §4.4.3](../hypervisor/hardware-spec.md), which makes it unreachable), the hyperprivileged-register trap `0x1F0` at `VBR_HYP + 0x300` ([aic/aic2-spec.md §5.1](../aic/aic2-spec.md)), silently dropped ([§5.6](../aic/aic2-spec.md)), and dropped with `SLVERR` at the `AIC2_LOCK` BMID filter ([§6.3](../aic/aic2-spec.md)) — and §4.4.3 says the other three describe a case that cannot arise. **(b)** Three injection mechanisms coexist with no arbitration: §5.2's hardware fast path, `VINJ_SEND` ([§5.3](../aic/aic2-spec.md)), and `HCALL_HV_INTR_INJECT` ([hypervisor/linux-spec.md §3.4](../hypervisor/linux-spec.md)), the last of which appears in no hypercall list in [hypervisor/design-spec.md](../hypervisor/design-spec.md). **(c)** Two hypercall names — `HCALL_HV_IPI_SEND` ([aic/aic2-spec.md §5.6](../aic/aic2-spec.md)) and `HCALL_HV_VIRTIO_NOTIFY` ([hypervisor/linux-spec.md §6](../hypervisor/linux-spec.md)) — appear in prose and in **no** service table, so they have no number, no arguments and no validation obligation | [aic/aic2-spec.md](../aic/aic2-spec.md) for (a) and (b); [hypervisor/linux-spec.md §3.4](../hypervisor/linux-spec.md) for (c) |
