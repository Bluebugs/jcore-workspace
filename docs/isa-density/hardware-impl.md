...
- `movi20s`: `imm_val = imm20 << 8`, sign-extended from bit **27** of the shifted value — [`spec.md`](spec.md) §3.1 owns that notation.
  <br>This bullet previously read `sign_extend_from_bit19(imm20) << 8` and
  deferred to a §8.5 open question. The two expressions denote the same
  function on every immediate; the question is closed and the wording follows
  the owner so the tree carries one notation. *(Kept on its own line: a
  retraction phrase sharing a line with the live value would exempt that line
  from `no-stale-value` for good.)*
