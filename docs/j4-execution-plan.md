...
| C2d | IOMMU default-deny + per-device block + no global-match IOTLB + coherent-DMA owner. **Design DONE 2026-09-09** — [iommu/hardware-spec.md §3.10](iommu/hardware-spec.md) `I-R1`–`I-R10` and §10.1 `I-E0`–`I-E6`, [iommu/security-review.md](iommu/security-review.md) (commissioned by this task; it did not exist), [decisions/0010](decisions/0010-dma-coherence-is-software-maintained.md), [security/threat-model.md §7.7a, §8 L2](security/threat-model.md). **The per-device block state already existed on both sides of the interface — *but only one of those sides is code*, and this cell said so in a way a reader takes as RTL *(corrected post-F, 2026-09-10, per Task F §5.2)*: the kernel side is real generic IOMMU code, the hardware side is a specification document, and there is no IOMMU in either RTL repository. The prose below this table always said that; the cell compressed it into a claim it could not carry.** **The coherent-DMA entry is wrong about every noun, and the implementation half is not dispatchable in any of the three repos** — see below. | docs → jcore-cpu + jcore-soc + linux | Opus | *blocked on a BMID-carrying bus and a DMA master existing* |
| C2e | Cache isolation beyond ways (DAWG-semantics metadata + MSHR reservation + bandwidth QoS + per-tenant KSM + privileged flush ops). **Design DONE 2026-09-09** — [cache/l2-spec.md §16.2](cache/l2-spec.md) [`P-R1`–`P-R8`](cache/l2-spec.md), §16.3, §16.4 `P-E1`–`P-E5`, §22.1b, [hypervisor/design-spec.md §6.2](hypervisor/design-spec.md), [security/threat-model.md §7.6a, §8 L5, §8 L6](security/threat-model.md). **The named mechanism has no pre-2006 grounding and was re-derived rather than adopted, one of the five was asked about the wrong object, one was already discharged by a section that existed, and the implementation half is not dispatchable in either repo** — see below. **A sixth defect was invisible from inside this task and was found by F reading it against C2c: the partition was keyed on `PDID`, which is optional and absent on the in-order default path [0009](decisions/0009-in-order-fgmt-is-the-default-path.md) chose, so building the L2 would not have moved L5. Carrier corrected to `HTCR.TENANT` by row G.** | docs → jcore-cpu + linux | Opus | *blocked on an L2 existing* |
...
six macros to `do { } while (0)`: `save_fpu`, `restore_fpu`, `release_fpu`, `grab_fpu`,
`fpu_state_restore` and `__fpu_state_restore` *(the sixth added post-F, 2026-09-10 — Task F §5.2
found the count was five here and six in the header)*.
