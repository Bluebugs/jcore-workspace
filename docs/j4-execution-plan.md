...
| # | Task | Repo(s) | Model | Review |
|---|---|---|---|---|
| B0a | "One authority per fact" decision + supersede-header convention; apply supersede headers to known-stale sections. | docs | **Opus** (decision) → **Sonnet** (apply) | Opus design-review |
...
| B0c | Doc-vs-code CI checks (page size, TSB offsets, P4 offsets, context image sizes) + `insns2asm --emit check` + `freespace` collision sweep in CI. | jcore-cpu + docs | **Sonnet** (scripts) / **Opus** (what to assert) | Opus code |
